...
#include "Model/s21_objparser.h"

s21_MainWindow::s21_MainWindow(QWidget *parent)
    : QMainWindow(parent), ui_(new Ui::s21_MainWindow) {
  ui_->setupUi(this);
  ui_->Screenshot->hide();
  ui_->Gif->hide();
  ui_->SettingsTab->tabBar()->hide();

  joy_stick_ = new s21::JoyStick(this);
  s21::Controller::SetJoystick(joy_stick_);
  joy_stick_->setStyleSheet(
...
void s21_MainWindow::keyReleaseEvent(QKeyEvent *event) { z_pressed_ = false; }
